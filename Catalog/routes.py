...
@app.route("/home")
def home():
    categorieitems = CategorieItem.query.order_by(
        CategorieItem.created_at.desc()).all()
...
def get_Item_via_Categories(name):
    ...
    if categorie is not None:
        items = CategorieItem.query.filter_by(
            categorie_id=categorie.id).order_by(CategorieItem.name)
        return jsonify(CategorieItem=[i.serialize for i in items])
    else:
...
def newItem():
    ...
    if form.validate_on_submit():
        ...
        item = CategorieItem(name=form.name.data,
                             description=form.description.data,
                             author=current_user,
                             price=str(form.price.data[0]),
                             categorie=Categorie.query.get_or_404(
                                 form.categorie.data),
                             picture=filename)
        db.session.add(item)
        db.session.commit()
